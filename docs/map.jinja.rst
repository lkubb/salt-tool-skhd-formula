...
We will see a quick setup to configure the ``tool_skhd`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_skhd`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_skhd`` formulas under ``/srv/salt/tool_skhd/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_skhd/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_skhd/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_skhd/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_skhd/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_skhd`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_skhd`` formulas under ``/srv/salt/tool_skhd/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_skhd/parameters/roles
...
We create a configuration for the role ``tool_skhd/server`` in ``/srv/salt/tool_skhd/parameters/roles/tool_skhd/server.yaml``:
...
We create another configuration for the role ``tool_skhd/client`` in ``/srv/salt/tool_skhd/parameters/roles/tool_skhd/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_skhd/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_skhd/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_skhd/parameters/dns:domain/  '/srv/salt/tool_skhd/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-skhd-as-json:
      test.nop:
        - name: {{ skhd | json }}
...
.. code-block:: sls
    ...
    skhd-config-file-file-managed:
      file.managed:
        - name: {{ skhd.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='skhd-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ skhd.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``skhd`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ skhd['winner'] }}
